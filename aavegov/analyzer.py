...
def getConfig_history_pd(entry):
    ...
    config_history[index_field] = [date.fromtimestamp(
        w) for w in config_history['timestamp']]
    config_history_pd = config_history[
        index_field + config_fields
    ].drop_duplicates(subset=index_field, keep='first').set_index(index_field).astype(int)
...
def getConfig(entry):
    ...
    config_current.index = [
        date.fromtimestamp(entry['lastUpdateTimestamp'])]

    config_pd = config_current.append(
        config_history_pd).sort_index(ascending=True)
    return config_pd
...
def plotConfig(entry, figdim=(7, 5)):
    ...
    for w in config_fields:
        ...
        ax1.plot(datapoints, drawstyle='steps-post',
                 label=config_legend[w], marker='o', alpha=0.8)
        valign = 'top' if w == 'baseLTVasCollateral' else 'bottom'
        for i in range(len(datapoints)):
            ax1.annotate(datapoints[i],
    ...
    ax3.bar(pricedata_pd.index, pricedata_pd['volumeto'],
            label='daily volume',
    ...
    } for w in reserve_hist if w['reserve']['symbol'] == symb]
    )

    ax4.plot(
        reserve_hist_coin['Date'],
        reserve_hist_coin['utilization'],
        label='actual utilisation', alpha=0.7, c='red')
    ax4.axhline(
        y=int(entry['optimalUtilisationRate'])/1e27,
        label='optimal utilisation', alpha=0.7, c='olive')
    ax4.set_ylim([0, 1])
    ...
    reserve_hist_coin.plot.area(
        x='Date', y=['total borrows', 'total liquidity'],
        ax=ax5, stacked=False, legend=False)
...
if __name__ == 'main':
    ...
    config_fields = ['reserveLiquidationBonus',
                     'reserveLiquidationThreshold', 'baseLTVasCollateral']

    # define corresponding legend label for plot
    config_legend = Series(
        ['liquidation incentive', 'liquidation threshold', 'collateral factor'], index=config_fields)
    ...
    colname.update({'std': '30-day volatility',
                    'vol': '30-day average volume'})
    risk_relation_df = risk_relation_df.rename(colname, axis=1)
    risk_relation_corr = risk_relation_df.corr(method='spearman')
    pval = risk_relation_df.corr(method=lambda x, y: spearmanr(x, y)[
        1]) - eye(*risk_relation_corr.shape)
    p = pval.applymap(lambda x: ''.join(
        ['*' for t in [0.01, 0.05, 0.1] if x <= t]))

    plt.close()

    heatmap(risk_relation_corr,
            vmin=-1,
            vmax=1,
