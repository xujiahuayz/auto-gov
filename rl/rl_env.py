...
class LendingProtocolEnv(gym.Env):
    def __init__(self, market):
        ...
        self.observation_space = gym.spaces.Box(low=np.array([0, 0, 0, 0, 0]), high=np.array([1, np.inf, np.inf, np.inf, np.inf]), dtype=np.float32)
...
class Market:
    ...
    def get_state(self):
        return np.array([self.utilization_rate, self.total_supply, self.liquidation_threshold, self.liquidation_discount_factor, self.collateral_factor])
